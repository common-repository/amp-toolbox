...
if (!class_exists('WDPanelAdmin2')) {
	class WDPanelAdmin2 {
		...
		function add_ozh_adminmenu_icon( $hook ) {
			if ($hook == $this->hook) 
				return plugins_url( '/'.$this->ozhicon, __FILE__ );
			return $hook;
		}
		...
		function config_page_styles() {
			if (isset($_GET['page']) && $_GET['page'] == $this->hook) {
				...
				wp_enqueue_style('blogicons-admin-css', plugins_url( '/WDPanelAdmin.css', __FILE__ ));
			}
		}
		...
		function form_table($rows) {
			...
			foreach ($rows as $row) {
				$content .= '<tr id="tr_'.$row['id'].'"><th valign="top" scrope="row">';
		...
		function plugin_support() {
			$content = '<p>'.__('If you have any problems with this plugin or good ideas for improvements or new features, please talk about them in the','schemaplugin').' <a href="http://wordpress.org/extend/plugins/'.$this->hook.'" target="_blank">'.__("Support forums",'schemaplugin').'</a>.</p>';
		...
		function wd_news() {
			...
			$rssobj = fetch_feed( 'https://webdesires.co.uk/feed/' );
			
			if ( ! is_wp_error( $rssobj ) ) {
				...
				$this->postbox('schemalatest', 'Latest from WebDesires Blog', 'Couldn\'t fetch feed... This may be a temporary error.');
			}
		}
		function wd_knowledge() { 
			include_once( ABSPATH . WPINC . '/feed.php' );
			$rssobj = fetch_feed( 'https://webdesires.co.uk/feed/?post_type=knowledge_base' );
